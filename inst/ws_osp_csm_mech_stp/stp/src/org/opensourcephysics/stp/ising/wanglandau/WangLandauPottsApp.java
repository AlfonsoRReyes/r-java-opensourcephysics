...
public class WangLandauPottsApp extends AbstractSimulation {
  PlotFrame histogramFrame = new PlotFrame("E", "H(E)", "Histogram"),
            ...
            pEnergyFrame = new PlotFrame("E", "ln P(E)", "Energy Distribution (Not normalized)");
  ...
  int dE(int i, int newSpin) {
    ...
    int sum = 0;
    if(spin[i] == spin[u])sum++;
    if(spin[i] == spin[d])sum++;
    if(spin[i] == spin[l])sum++;
    if(spin[i] == spin[r])sum++;
    if(newSpin == spin[u])sum--;
    if(newSpin == spin[d])sum--;
    if(newSpin == spin[l])sum--;
    if(newSpin == spin[r])sum--;
       return sum;
  }
  ...
  void flipSpins() {
    for(int steps = 0; steps<N; steps++) {
      ...
      int newSpin = (spin[i] + (int)(1 + Math.random()*(q-1))) % q;
      int delE = dE(i,newSpin);
      if(Math.random()<Math.exp(g[E]-g[E+delE])) {
        spin[i] = newSpin;
        E += delE;
      }
      g[E] += Math.log(f);
  ...
  public void reset() {
	...
	control.setValue("q", 10);
	control.setAdjustableValue("T for P(E)", 0.7);
    variable_initialize();
  ...
  public static void main(String[] args) {
    SimulationControl.createApp(new WangLandauPottsApp(), args);
  }
