...
public class XYModel implements Measurable {
  ...
  /**
   * so one MC styep per spin according to the Boltzman Probability
   * e^(-beta*DeltaEnergy)
   */
  ...
  public double getMeanEnergySquared() {
    return energySquaredAccumulator*1.0/((double) time);
  }

  public double getMeanMagnetizationX() {
    return magnetizationXAccumulator/time;
  }

  public double getMeanMagnetizationY() {
    return magnetizationYAccumulator/time;
  }
  ...
  public double getMeanMagnetizationSquared() {
    return magnetizationSquaredAccumulator*(1.0/(double) time);
  }
